    ...
    renderCell: ({ row }) => {
      return (
        <div className="w-full h-full items-center flex justify-center">
          <span className={`${renderStatusColor(row.status)} text-sm`}>
            {row.status}
          </span>
        </div>
      );
...
const rows2 = (): any[] => {
  ...
  loopArray.forEach((num) => {
    ...
    returnArray.push({
      ...
      status:
        statusPicker === 1 ? "Resolved" : statusPicker === 2 ? "Pending" : "",
    });
  });
  return returnArray;
};
...
export default function TransactionHistory({ customer }: { customer: string }) {
  ...
  return (
    <div className="w-full h-full overflow-y-auto flex flex-col custom-scrollbar pb-10 bg-[#F5F5F5]">
      ...
      <div className="px-10 w-full mt-4 flex flex-col">
